...
#GENERATE BAIL REQUESTS
    ...
    Nt_d = [0]
    last_t = 0
    for i in range(1, len(lam_d)):
        print(len(lam_d))
        if (lam_d[i] > 0):
            Na_d, Nt_d = gen_poisson_process(lam_d[i], inc)
            for j in range(len(Na_d)):
                donations.append(donation(Nt_d[j] + last_t , don_amt))
        last_t = i*inc#donations[len(donations)-1].t
        #print(donations[j].d)
    temp_don_count = np.zeros(len(donations))
    for i in range(len(donations)):
        print(donations[i].t)
        temp_don_count[i] = np.floor(donations[i].t)
    unique, counts = np.unique(temp_don_count, return_counts=True)
